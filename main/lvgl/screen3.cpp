...
void lvgl_screen3()
{
    ...
    lv_obj_set_pos(right_pane, LV_PCT(69), 100);
    lv_obj_set_size(left_pane, LV_PCT(69), LV_PCT(100));
    lv_obj_set_size(right_pane, LV_PCT(31), LV_PCT(100));
    ...
    for (size_t k = 0; k < sizeof(models_id) / sizeof(uint16_t); k++)
    {
        ...
        lv_label_set_text_fmt(lbl, "On/Off: 0x%04X", models_id[k]);
        auto click = [](lv_event_t* ev)
        {
            auto addr = (uint32_t)lv_event_get_user_data(ev);
            model_1000_modal_action(right_pane, addr);
        };
        ...
        for (size_t j = 0; j < comp.element_num; j++)
        {
            ...
            for (size_t k = 0; k < elem.count; k++)
            {
                if(elem.models[k].mod_id == 0x1000)
                {
                    auto btn = lv_button_create(cont_row);
                    auto lbl = lv_label_create(btn);
                    lv_label_set_text_fmt(lbl, "On/Off: 0x%04X", addr);
                    auto click = [](lv_event_t* ev)
                    {
                        auto addr = (uint32_t)lv_event_get_user_data(ev);
                        model_1000_modal_action(right_pane, addr);
                    };
                    lv_obj_add_event(btn, click, LV_EVENT_SHORT_CLICKED, (void*)(addr + j));
                }
            }
        }        
    }
