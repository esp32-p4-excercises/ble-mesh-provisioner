...
cJSON *JSON::add(const char *key, float val)
{
    int v1 = val * 100;
    float _val = v1 / 100;
    return cJSON_AddNumberToObject(root, key, _val);
}
...
cJSON *JSON::add(const char *key, long val)
{
    return cJSON_AddNumberToObject(root, key, (double)val);
}
...
bool JSON::appendToArray(cJSON *obj)
{
    return cJSON_AddItemToArray(root, obj);
}

bool JSON::appendToArray(float val)
{
    float _val = (int)(val * 1000 + .5);
    
    cJSON* obj = cJSON_CreateNumber(_val / 1000.0);
    return appendToArray(obj);
}
...
bool JSON::hasKey(const char *key)
{
...
{
    if (cJSON_IsNumber(root)) return cJSON_GetNumberValue(root);
