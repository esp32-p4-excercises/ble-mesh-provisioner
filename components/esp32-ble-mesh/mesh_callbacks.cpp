...
void _default_ble_mesh_provisioning_cb(esp_ble_mesh_prov_cb_event_t event, esp_ble_mesh_prov_cb_param_t *params)
{
    ...
    switch (event)
    {
        ...
        case ESP_BLE_MESH_PROVISIONER_ADD_LOCAL_APP_KEY_COMP_EVT:{
            printf("add APP key: %d\n", params->provisioner_add_app_key_comp.app_idx);
            break;
        }
        case ESP_BLE_MESH_PROXY_CLIENT_RECV_ADV_PKT_EVT:{
            // printf("\t\tNet IDX: %d, %i\n", params->proxy_client_recv_adv_pkt.net_idx, params->proxy_client_recv_adv_pkt.rssi);
            break;
        }
        default:
            break;
...
void _default_ble_mesh_config_client_cb(esp_ble_mesh_cfg_client_cb_event_t event, esp_ble_mesh_cfg_client_cb_param_t *params)
{
    ...
    ESP_LOGI(TAG, "%s, error_code = 0x%02x, event = 0x%02x, addr: 0x%04x, opcode: 0x%04x, model: 0x%04x",
             __func__, params->error_code, event, params->params->ctx.addr, opcode, params->params->model);
    ...
    if (device)
    {
        auto model = (BLEmeshConfigCli*)device->findModel(params->params->model);
        if (model)
